...
const generateContent = async (prompt, maxRetries = 3) => {
  ...
  console.log(
    `🔑 API Key status: ${
      GEMINI_API_KEY ? `${GEMINI_API_KEY.substring(0, 10)}...` : "MISSING"
    }`
  );
  ...
  if (!GEMINI_API_KEY) {
    return {
      success: false,
      message:
        "GEMINI_API_KEY is missing from environment variables. Please set it in .env file.",
      data: null,
      error: {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.error(
        `❌ ${GEMINI_MODEL} Error (attempt ${attempt}/${maxRetries}):`,
        {
