...
function buildPrompt({ topicName, level, goal, include }) {
  ...
  return `Bạn là chuyên gia triết học Mác-LêNin. Hãy tạo gói học liệu ngắn gọn, CHỈ JSON, tiếng Việt, về chủ đề: "${
    topicName || "Triết học Mác-LêNin"
  }" cho level: "${level || "intermediate"}"${
    goal ? `, mục tiêu: "${goal}"` : ""
  }.

🚨 CRITICAL: Title PHẢI LÀ CHÍNH XÁC: "${
    topicName || "Triết học Mác-LêNin"
  }" - KHÔNG ĐƯỢC THAY ĐỔI!

YÊU CẦU NGHIÊM NGẶT:
- CHỈ về triết học Mác-LêNin (KHÔNG phải kinh tế chính trị).
...
FORMAT JSON:
{
  ...
  ${
    wants.mindmap
      ? '"mindmapNodes": [{"id":"root","label":"string","children":[{"id":"","label":"","children":[]}]}],'
      : ""
  }
  ${
    wants.flashcards
      ? '"flashcards": [{"term":"string","definition":"string","example":"string","tags":["string"],"difficulty":"easy|medium|hard"}],'
      : ""
  }
  ${
...
async function generateContentPackInternal(params) {
  ...
  console.log(
    `🤖 Generating content pack with Multi-AI for topic: ${params.topicName}`
  );
  ...
  console.log(
    `🔍 AI returned title: ${data.title}, forcing to: ${params.topicName}`
  );
