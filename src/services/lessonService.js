...
const getSkills = async (userId, topic, level) => {
    try {
        ...
        if (userId && topic && level) {
            ...
            if (user.level === 'beginner' && !user.completedBasicVocab.includes(topic)) {
...
const createLesson = async (lessonData) => {
    try {
        if (!lessonData.title || !lessonData.type || !lessonData.topic || !lessonData.level || !lessonData.skill || !lessonData.questions) {
            return {
                success: false,
                statusCode: 400,
        ...
        const lesson = await Lesson.create({
            ...
            maxScore: lessonData.level === 'beginner' ? 1000 : lessonData.level === 'intermediate' ? 1500 : 2000,
...
const getLessons = async (userId, queryParams) => {
    try {
        ...
        if (userId) {
            ...
            if (user.level === 'beginner' && topic && !user.completedBasicVocab.includes(topic)) {
...
const completeLesson = async (userId, lessonId, score, questionResults, isRetried = false) => {
    try {
        ...
        if (user.level === 'beginner' && lesson.skill !== 'vocabulary' && !user.completedBasicVocab.includes(lesson.topic)) {
            console.log(`User ${userId} is beginner and has not completed vocab for topic ${lesson.topic}`);
            return {
                success: false,
                statusCode: 403,
        ...
        if (user.level === 'beginner' && lesson.skill === 'vocabulary') {
            if (!user.completedBasicVocab.includes(lesson.topic)) {
                ...
                console.log(`Added ${lesson.topic} to completedBasicVocab for user ${userId}`);
            }
        }
